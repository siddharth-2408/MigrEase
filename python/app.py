...
CONFIG = {
    "csv_path": "python/ikea.csv",
    "input_path": args.input_path,
    "cache_dir": "python/image_cache_ssim",
    "report_path": "python/report",
...
def generate_html_report(input_image_path, matched_data=None, fallback=None):
    ...
    category = CONFIG["target_category"].title()

    if fallback:
        dims = fallback
    elif matched_data:
        dims = {
            "width": matched_data["details"].get("width", "N/A"),
            "depth": matched_data["details"].get("depth", "N/A"),
            "height": matched_data["details"].get("height", "N/A"),
            "weight": matched_data["details"].get("weight", "N/A")
        }
    else:
        dims = {"width": "N/A", "depth": "N/A", "height": "N/A", "weight": "N/A"}
    ...
    <body>
      <h2>Estimated Dimensions for {category}</h2>
      <table>
        <tr><th>Width</th><td>{dims['width']} cm</td></tr>
        <tr><th>Depth</th><td>{dims['depth']} cm</td></tr>
        <tr><th>Height</th><td>{dims['height']} cm</td></tr>
        <tr><th>Weight</th><td>{dims['weight']} kg</td></tr>
      </table>
    </body>
    </html>
    """
